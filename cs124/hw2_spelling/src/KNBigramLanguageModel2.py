...
class KNBigramLanguageModel2:
    ...
    def getKnProbManually(self, bigram, detailed=False):
        wi_1, wi = bigram
        T1 = max(self.bigramCounts[bigram] - self.d, 0) / self.unigramCounts[wi_1]

        Pc = len([(v, w) for (v, w) in self.bigramCounts if w == wi]) / len(self.bigramCounts)
        norm = sum([self.bigramCounts[(v, w)] for (v, w) in self.bigramCounts if v == wi_1])
        lambd = self.d * len([(v, w) for (v, w) in self.bigramCounts if v == wi_1]) / norm
        T2 = lambd * Pc

        if detailed:
            return T1, Pc, lambd, T2
