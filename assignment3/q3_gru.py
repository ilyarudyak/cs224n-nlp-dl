...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        # returns list of tuples (gradient, variable)
        computed_grads_and_vars = optimizer.compute_gradients(loss)
        variables = [v for _, v in computed_grads_and_vars]
        computed_grads = [g for g, _ in computed_grads_and_vars]
        if self.config.clip_gradients:
            clipped_grads, _ = tf.clip_by_global_norm(t_list=computed_grads,
                                                      clip_norm=self.config.max_grad_norm)
            gradients = clipped_grads
        else:
            gradients = computed_grads
